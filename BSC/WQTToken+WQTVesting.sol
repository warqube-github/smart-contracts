...
contract WQTtoken is Context, IBEP20, Ownable {
  ...
  // Modifier
    ...
    function unstakeTokens(IBEP20 token, uint256 amount) public timestampIsSet noReentrant {
        require(_balances[msg.sender] >= amount, "Insufficient token balance, try lesser amount");
        ...
        if (block.timestamp >= timePeriod) {
            reward = ((block.timestamp - timePeriod) / 365 * amount * _ROI/100) + amount;
            alreadyWithdrawn[msg.sender] = alreadyWithdrawn[msg.sender].add(reward);
            _balances[msg.sender] = _balances[msg.sender].sub(reward);
            token.transfer(msg.sender, reward);
            emit TokensUnstaked(msg.sender, reward);
        } else {
    ...
    function transferAccidentallyLockedTokens(IBEP20 token, uint256 amount) public onlyOwner noReentrant {
        ...
        token.transfer(contractOwner, amount);
    }
  ...
  */
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    ...
    function addROI(uint256 ROI) public onlyOwner returns (bool){
        _ROI = ROI;
        return true;
    }

    function getROI() public onlyOwner view returns (uint256) {
        return _ROI;
    }
 ...
 * @title TokenVesting
 */
contract WQTVesting is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
